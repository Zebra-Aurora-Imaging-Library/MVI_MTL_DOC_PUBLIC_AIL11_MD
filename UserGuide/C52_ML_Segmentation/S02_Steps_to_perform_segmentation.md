...
section: Steps_to_perform_segmentation
module_tag: class
product: Aurora-Imaging-Library-user-guide
version: 1100
path: "UserGuide / Machine learning tasks / ML_Coarse_segmentation / Steps to perform segmentation"
---

# Steps to perform segmentation

The following steps provide a basic methodology to perform segmentation with the Aurora Imaging Library Classification module:

1. [Perform all required allocations](S02_Steps_to_perform_segmentation.md).
2. [Build and populate your dataset](S02_Steps_to_perform_segmentation.md).
3. [Train your classifier context](S02_Steps_to_perform_segmentation.md).
4. [Predict with your classifier](S02_Steps_to_perform_segmentation.md).
5. If necessary, [Save your classification contexts](S02_Steps_to_perform_segmentation.md).
6. [Free your allocated objects](S02_Steps_to_perform_segmentation.md).

## Perform all required allocations

These allocations are required to perform segmentation. In many cases, some of these allocations will be imported or restored from previous work, using [`MclassImport`](../../Reference/class/MclassImport.md) or [`MclassRestore`](../../Reference/class/MclassRestore.md).

1. Allocate a predefined segmentation classifier context, using [`MclassAlloc`](../../Reference/class/MclassAlloc.md) with [`M_CLASSIFIER_SEG_PREDEFINED`](../../Reference/class/MclassAlloc.md) and [`M_CSNET_...`](../../Reference/class/MclassAlloc.md). Note, [`MclassTrain`](../../Reference/class/MclassTrain.md) can automatically allocate a classifier context if none is specified.
...
3. Allocate a segmentation training context, using [`MclassAlloc`](../../Reference/class/MclassAlloc.md) with [`M_TRAIN_SEG`](../../Reference/class/MclassAlloc.md). A training context holds the settings with which to train a classifier context.
4. Allocate a segmentation training result buffer to hold training results, using [`MclassAllocResult`](../../Reference/class/MclassAllocResult.md) with [`M_TRAIN_SEG_RESULT`](../../Reference/class/MclassAllocResult.md).
5. Allocate a segmentation result buffer to hold the prediction results, using [`MclassAllocResult`](../../Reference/class/MclassAllocResult.md) with [`M_PREDICT_SEG_RESULT`](../../Reference/class/MclassAllocResult.md).

Object allocation for segmentation is summarized in the table below:

| *[Image: MclassObject.png]* | Aurora Imaging Library allocation |
| --- | --- |
| Classifier context | [`M_CLASSIFIER_SEG_PREDEFINED`](../../Reference/class/MclassAlloc.md) |
| Specific predefined classifier context | [`M_CSNET_...`](../../Reference/class/MclassAlloc.md) |
| Dataset context | [`M_DATASET_IMAGES`](../../Reference/class/MclassAlloc.md) |
| Data preparation context | [`M_PREPARE_IMAGES_SEG`](../../Reference/class/MclassAlloc.md) |
| Training context | [`M_TRAIN_SEG`](../../Reference/class/MclassAlloc.md) |
| Training result buffer | [`M_TRAIN_SEG_RESULT`](../../Reference/class/MclassAllocResult.md) |
| Prediction result buffer | [`M_PREDICT_SEG_RESULT`](../../Reference/class/MclassAllocResult.md) |
...
1. Add class definitions to the source dataset, using [`MclassControl`](../../Reference/class/MclassControl.md) with [`M_CLASS_ADD`](../../Reference/class/MclassControl.md). Note, the number of classes with which to categorize your data is a key decision to make when performing segmentation.
...
3. For each entry, add regions and specify the class definitions (ground truths) that are represented, using [`MclassEntryAddRegion`](../../Reference/class/MclassEntryAddRegion.md) with the [`ClassIndexGroundTruth`](../../Reference/class/MclassEntryAddRegion.md) parameter. This step is known as labeling your data.
...
1. Optionally, set the target image size, using [`MclassControl`](../../Reference/class/MclassControl.md) with [`M_TARGET_IMAGE_SIZE_X`](../../Reference/class/MclassControl.md) and [`M_TARGET_IMAGE_SIZE_Y`](../../Reference/class/MclassControl.md).
...
3. Perform the prediction operation with the trained classifier context and the target data that you want to classify, using [`MclassPredict`](../../Reference/class/MclassPredict.md). If your training images were prepared, then the target image must also be prepared in the same way. This maintains consistency between the training data and the target data.
   Optionally, you can perform the prediction operation with a dataset as your target. For segmentation, you must set the [`M_SEGMENTATION_FOLDER`](../../Reference/class/MclassControl.md) control for the target dataset. Segmentation prediction scores are saved at this location. When predicting with a dataset as your target, you can hook functions to prediction events, using [`MclassHookFunction`](../../Reference/class/MclassHookFunction.md).
...
4. Retrieve the required results from the prediction result buffer, using [`MclassGetResult`](../../Reference/class/MclassGetResult.md). You can also draw prediction results, using [`MclassDraw`](../../Reference/class/MclassDraw.md).
