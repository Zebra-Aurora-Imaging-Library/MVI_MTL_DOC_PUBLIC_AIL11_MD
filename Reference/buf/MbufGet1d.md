...
This function copies data from a specified one-dimensional area of an Aurora Imaging Library source buffer to a user-supplied array. If the source buffer has multiple bands, this function linearly copies the data from the one-dimensional area of each band. The internal data format of the source buffer need not match the data format of the user-supplied array (planar); an internal conversion will be performed if necessary. Note, however, if the formats do match, the operation will be much faster. For more information, refer to [`MbufGetColor2d`](../../Reference/buf/MbufGetColor2d.md) with [`M_PLANAR`](../../Reference/buf/MbufGetColor2d.md).
