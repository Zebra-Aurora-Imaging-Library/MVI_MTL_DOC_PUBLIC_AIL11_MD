...
doctype: Reference
module: sys
function: MsysInquireFeature
product: Aurora-Imaging-Library-reference
preliminary: true
version: 1100
path: "Reference / sys / MsysInquireFeature"
---
...
| `"Root"` | Specifies to inquire the highest-level feature of the XML structure; this feature is of type category and typically, its subfeatures are also of type category. Note that this value is only available when using [`M_SUBFEATURE_...`](../../Reference/sys/MsysInquireFeature.md). |

### `UserVarType` *(in, AIL_INT64)*

Specifies the data type of the address pointed to by the [`UserVarPtr`](../../Reference/sys/MsysInquireFeature.md)parameter. If not inquiring the feature's value (using [`M_FEATURE_VALUE`](../../Reference/sys/MsysInquireFeature.md)), set this parameter to `M_DEFAULT`.

*For specifying the UserVarPtr's data type*

| Value | Description |
| --- | --- |
| `M_TYPE_BOOLEAN` | Specifies that [`UserVarPtr`](../../Reference/sys/MsysInquireFeature.md) is passed an address of type_AIL_BOOL_. |
| `M_TYPE_DOUBLE` | Specifies that [`UserVarPtr`](../../Reference/sys/MsysInquireFeature.md) is passed an address of type _AIL_DOUBLE_. |
| `M_TYPE_INT64` | Specifies that [`UserVarPtr`](../../Reference/sys/MsysInquireFeature.md) is passed an address of type _AIL_INT64_. |
| `M_TYPE_STRING` | Specifies that [`UserVarPtr`](../../Reference/sys/MsysInquireFeature.md) is passed an address of type _AIL_TEXT_CHAR_. |
| `M_TYPE_UINT8` | Specifies that [`UserVarPtr`](../../Reference/sys/MsysInquireFeature.md) is passed an address of type _AIL_UINT8_. |
...
Inquires whether an event occurs when the value of the specified feature changes. If enabled, [`MsysHookFunction`](../../Reference/sys/MsysHookFunction.md) with [`M_FEATURE_CHANGE`](../../Reference/sys/MsysHookFunction.md) hooks a specified function to the feature change event.
...
Inquires the specified feature's display name. Note that this can differ slightly from the internally-used feature name ([`M_FEATURE_NAME`](../../Reference/sys/MsysInquireFeature.md)).
...
Inquires the specified internally-used feature name. Note that this can differ slightly from the displayed name of the feature ([`M_FEATURE_DISPLAY_NAME`](../../Reference/sys/MsysInquireFeature.md)).
...
Inquires whether the specified feature is present in the camera's device description file. Note that, if the feature is listed in the camera's device description file (XML), use[`M_FEATURE_ACCESS_MODE`](../../Reference/sys/MsysInquireFeature.md) and the **M_FEATURE_IS_IMPLEMENTED** macro to determine if the feature is supported. If the feature is not present in the file, [`M_FEATURE_PRESENT`](../../Reference/sys/MsysInquireFeature.md) will return [`M_NO`](../../Reference/sys/MsysInquireFeature.md), whereas [`M_FEATURE_ACCESS_MODE`](../../Reference/sys/MsysInquireFeature.md) will generate an error.
...
Inquires the display format for the feature's value. The display format dictates the size of the field and its form (for example, check box, slider, text-box) used in a user interface for the camera's device description file (XML), such as Feature Browser (accessible through Aurora Imaging Intellicam or[`MsysControl`](../../Reference/sys/MsysControl.md) with [`M_GC_FEATURE_BROWSER`](../../Reference/dig/MdigControl.md)).  Note that this inquire type can only be used with a feature that has a data type of _AIL_INT64_ or _AIL_DOUBLE_.
...
Inquires the number of elements in the specified feature's array of values. For use with [`M_FEATURE_VALUE_ARRAY`](../../Reference/sys/MsysInquireFeature.md).
...
Inquires the internally used name of the specified enumeration entry, where _n_ is the index into the enumerated list.
...
Inquires the internally-used name of the specified enumeration entry of the feature, where _n_ is the index into the enumerated list. Note that this can differ slightly from the display name of the entry ([`M_FEATURE_ENUM_ENTRY_DISPLAY_NAME + n`](../../Reference/sys/MsysInquireFeature.md)).
...
When the specified feature is of type category or [`"Root"`](../../Reference/sys/MsysInquireFeature.md), you can use one of the following to inquire about its subfeatures. This is useful for enumerating the features of your camera.
...
Retrieves the length of the string, including the terminating null character ("\0"). Note that, when used with[`M_FEATURE_VALUE`](../../Reference/sys/MsysInquireFeature.md), you can only use this combination value if the feature is of type string.
