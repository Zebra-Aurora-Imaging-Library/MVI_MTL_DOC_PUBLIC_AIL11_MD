...
doctype: Reference
module: edge
function: MedgeRestore
product: Aurora-Imaging-Library-reference
version: 1100
path: "Reference / edge / MedgeRestore"
---
...
This function restores an Edge Finder context that was previously saved to a file, using [`MedgeSave`](../../Reference/edge/MedgeSave.md) or [`MedgeStream`](../../Reference/edge/MedgeStream.md). This function restores all the Edge Finder context's settings that were in effect when the Edge Finder context was saved.

When the restored Edge Finder context is no longer required, release it using[`MedgeFree`](../../Reference/edge/MedgeFree.md)unless [`M_UNIQUE_ID`](../../Reference/edge/MedgeRestore.md) was specified during allocation; if [`M_UNIQUE_ID`](../../Reference/edge/MedgeRestore.md) was specified, the smart identifier manages the restored Edge Finder context's lifetime and you must not manually free it.
...
| `"FileName"` | Specifies the drive, directory, and name of the file (for example, _"C:\mydirectory\myfile"_). Typically, Edge Finder context files have an MEF extension.
...
| `Address in which to write the contour Edge Finder context identifier` | Specifies the address of an _AIL_ID_ in which to write the identifier of the restored contour Edge Finder context.

If allocation fails, [`M_NULL`](../../Reference/edge/MedgeRestore.md) is written as the identifier. |
| `Address in which to write the crest Edge Finder context identifier` | Specifies the address of an _AIL_ID_ in which to write the identifier of the restored crest Edge Finder context.
...
The returned value is the Edge Finder context identifier either as a standard identifier (_AIL_ID_) or a smart identifier (_AIL_UNIQUE_EDGE_ID_). If allocation fails, `M_NULL` is returned (or _nullptr_ if[`M_UNIQUE_ID`](../../Reference/edge/MedgeRestore.md) was specified).
